...
import { UserJwtTy } from "../types/user.type";
...
export function decodedJWT(token: any, customErrMsg?: string) {
...
export function jwtGenerate(user: any) {
  ...
  const accessToken = jwt.sign(cleanUser, secretJWT, { expiresIn: "12h" });
  return accessToken;
}
